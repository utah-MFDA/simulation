        ...
        for f in files:
            ...
            for key, value in n.items():
                ...
                if key[-1] == 'c' or key[-1] == 'C':
                    replacement_list["V("+str(value)+")"] = chem_name + \
                        "("+"_".join(key.split("_")[:-1])+")"
                else:
                    replacement_list["V("+str(value)+")"] = "P(" + \
                        "_".join(key.split("_")[:-1])+")"
...
def main(config, ifile=None, ilist=None, wd="./", no_result_dir=False, debug=None):

    config_file = setConfig(config)
    print("Loaded config file:", config_file)
    if isinstance(config_file, str):
        sim = xyceSimulator(config_file)
    elif isinstance(config_file, list):
        sim = xyceSimulator(config_file[0])
    else:
        raise Exception(f"Not able to read config file {config_file}")

    infiles = parseFiles(ifile, ilist, wd)
    if ilist is not None:
        spiceList = pd.read_csv(wd+"/"+ilist)
    else:
        spiceList = None
...
if __name__ == "__main__":
    ...
    configDefault = os.path.dirname(os.path.normpath(
        os.path.realpath(__file__)))+"/xyceConfig"
