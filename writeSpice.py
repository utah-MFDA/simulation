...
def add_probes_to_device(probes, netlist_graph):
    ...
    if "pressure" in probes:
        ...
        for p in probes["pressure"]:
            ...
            dev_node = list(netlist_graph[probe_node].keys())[0]
            probe_list.append(f"V({probe_node}_{dev_node})")
    ...
    if "flow" in probes:
        for p in probes["flow"]:
            ...
            netlist_graph.remove_edge(p_nod, p_dev)
            flow_probe = f"vpr_{p_nod}_{p_dev}"
            new_probe_nodes = [
                (
                    f"{p_nod}_{p_dev}_pr",
                    {"node_type": "connection", "chem_wire": p_nod},
                ),
                (
                    f"{flow_probe}",
                    {"node_type": "flow_probe", "device": p_dev, "param": {"": "0"}},
                ),
            ]
            new_es = []
            new_es.append((new_probe_nodes[0][0], p_dev))
            new_es.append((new_probe_nodes[0][0], new_probe_nodes[1][0]))
            # new_es.append((new_probe_nodes[1][0], new_probe_nodes[2][0]))
            new_es.append((new_probe_nodes[1][0], p_nod))
    ...
    if "pressureNode" in probes:
        for p in probes["pressureNode"]:
            ...
            if isinstance(p, SimulationXyce.SimulationXyce.Probe):
                ...
                p_node = p[node]
                p_dev = p[dev]
            print("pressure node: ", p_node, p_dev)
            if p in probes["flow"]:
                # we don't need to edit the netlist after flow probes have been placed
                # probe_list.append(f"V({p[node]}_{p[dev]}_pr)")
                probe_list.append(f"V({p_node}_{p_dev}_pr)")
            else:
                netlist_graph.remove_edge(p_node, p_dev)
                new_probe_nodes = [
                    (
                        f"{p_node}_{p_dev}_pr",
                        {"node_type": "connection", "chem_wire": p[node]},
                    ),
                    (
                        f"vpr_{p_node}_{p_dev}",
                        {
                ...
                new_es.append((new_probe_nodes[0][0], new_probe_nodes[1][0]))
                # new_es.append((new_probe_nodes[1][0], new_probe_nodes[2][0]))
                new_es.append((new_probe_nodes[1][0], p_node))
    ...
    if "concentration" in probes:
        for p in probes["concentration"]:
            ...
            dev_node = list(netlist_graph[p_node].keys())[0]
            if f"V({p_node}_{dev_node}_chem)" not in probe_list:
                dev_node = list(netlist_graph[p_node].keys())[0]
                probe_list.append(f"V({p_node}_{dev_node}_chem)")
    ...
    if "concentrationNode" in probes:
        for p in probes["concentrationNode"]:
            ...
            netlist_graph.nodes[p_dev]["chem_connection"] = {
                "node": f"vchpr_{p_node}_{p_dev}_chem",
                "oth_node": p_node,
            }
            # netlist_graph.nodes[p.getNode()]["chem_connection"]={"node":f"vchpr_{p.getNode()}_conn_chem","oth_node":p.getDevice()}
            if (
                netlist_graph.nodes[p_node]["node_type"] in "input"
                and len(netlist_graph[p_node]) > 1
            ):
                chan_node = f"{p_node}_channel_out_chem "
            elif (
                netlist_graph.nodes[p_node]["node_type"] == "output"
                and len(netlist_graph[p_node]) > 1
            ):
                chan_node = f"{p_node}_channel_in_chem "
            elif (
                netlist_graph.nodes[p_node]["node_type"] == "wire"
                and len(netlist_graph[p_node]) > 2
            ):
                # which port
                if netlist_graph.edges[p_dev, p_node]["port"] == "out_fluid":
                    chan_node = f"{p_node}_channel_in "
                else:
                    chan_node = f"{p_node}_channel_out "
            elif "chem_connection" in netlist_graph.nodes[p_node]:
                chan_node = f"vchpr_{p_node}_conn_chem"
                netlist_graph.nodes[p_node]["chem_connection"] = {
                    "node": chan_node,
                    "oth_node": p_dev,
                }
            else:
            ...
            new_probe_node = [
                (
                    f"vchpr_{p_node}_{p_dev}_pr",
                    {
                        "node_type": "concentration_probe",
                        "chem_pr_wires": [
                            f"vchpr_{p_node}_{p_dev}_chem",
                            chan_node,
                        ],
                    },
                ),
                ...
                if len(in_netlist[node]) == 1:
                    if "chem_connection" in in_netlist.nodes[node]:
                        ...
                        if (
                            ...
                            l1_conn_node = list(in_netlist[conn_node])[0]
                            print(f"probe node {l1_conn_node}")
                            l2_conn_node = list(in_netlist[l1_conn_node])[0]
                            print(f"probe node {l2_conn_node}")
                            chem_nodes += f"{node}_{l2_conn_node}_chem "
                        else:
